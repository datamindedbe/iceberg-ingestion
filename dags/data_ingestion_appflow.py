...
def ingestion_operator(dag: DAG, source: str, table: str, ingest_type: str) -> ConveyorContainerOperatorV2:
    return ConveyorContainerOperatorV2(
        dag=dag,
        task_id=f"{source}-{table}-ingestion",
        instance_type="mx.small",
        cmds=["./scripts/appflow", region, source, table, env, date_path, ingest_type],
    )


def cleaning_operator(dag: DAG, source: str, table: str, ingest_type: str, dataset:Dataset, keep_deletes: bool,
...
config_path = os.path.join(os.path.dirname(dag.fileloc), "appflow_flows.yaml")
with open(config_path, 'r') as appflow_config:
    config = yaml.safe_load(appflow_config)

# Generate the ingestion and cleaning jobs for all flows in the configuration
for source, tables in config.items():
    dag = DAG(
        f"data_ingestion_appflow_{source}",
        default_args=default_args,
        schedule_interval="0 2 * * *",
        max_active_runs=1,
        tags=["data_ingestion"],
    ...
    for table, config in tables.items():
        ...
        keep_deletes = bool(config.get("keep_deletes", False))

        ingestion = ingestion_operator(dag=dag, source=source, table=table, ingest_type=ingest_type)
